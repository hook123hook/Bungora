...
async function rpcCall(rpcUrl, auth, method, params = {}) {
  const res = await fetch(rpcUrl, {
    ...
    body: JSON.stringify({ jsonrpc: '2.0', id: 'Bungora', method, params })
  });
  return res.json();
}
...
async function main() {
  let exitCode = 0;
  console.log('');
  console.log('Bungora Monero köprüsü — kurulum kontrolü');
  console.log('----------------------------------------');
  ...
  if (!supabaseUrl || !key || supabaseUrl.includes('YOUR-PROJECT-REF')) {
